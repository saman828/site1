$(document).ready(function () {
    ...
    $(document).find('input[data-name="cellphone"]').on('input', function () {
        ...
        if (simType !== '') {
            ...
            if ($.inArray($('#dataType').val(), ['MTN!', 'MTN#', 'WiMax']) === -1 && kind !== 'Pin') {
                $('#dataType').val(simType.toUpperCase());
                $('#dataSimType').val(simType.toUpperCase());
            }
        }
        if (kind !== 'Pin') {
            setPurchaseOptions();
    ...
    // });
    $('.purchase-payment-gateways').on('click', 'img', function () {
        $('.purchase-payment-gateways').find('img').removeClass('active');
        $(this).addClass('active');
    ...
    $('.purchase-payment-btn').on('click', function () {
        ...
        if (sendForm) {
            ...
            $.ajax({
                type: 'POST',
                url: '/purchase',
                data: $('form#myForm').serialize() + "&action=" + action,
                async: true,
                success: function (data) {
                    $('.loading h4').text('انتقال به بانک ...');
                    doProccess(JSON.parse(data));
                },
                error: function (e) {
                    $('.loading').fadeOut();
                }
